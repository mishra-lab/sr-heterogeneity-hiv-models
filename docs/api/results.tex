...
\label{sss:res:kp}
FSW were explicitly modelled in \x{kp/n.FSW.named} studies.
...
Models expliciltly modelled men who have sex with men (MSM)
in \x{kp/n.MSM} studies;
...
plus adjusted effect estimates for each factor from multivariate analysis.  %SM: suggest we revise terminology a bit to say 'associated with' rather than "reduced/increased"
...
Compared with zero risk heterogenetiy, including risk heterogeneity with activity groups but without key populations was associated with
slightly higher impacts ---adjusted effect (95\% CI):
\x{api/inc/Risk.Activity-(no-KP).eff}\% IR, \x{api/chi/Risk.Activity-(no-KP).eff}\% CIA.
Scenarios that included key population(s) and assumed similar ART cascade between groups were 
generally associated with a slightly larger impact compared with models without risk 
hetrogeneity: \x{api/inc/Risk.KP-(same).eff}\% IR, \x{api/chi/Risk.KP-(same).eff}\% CIA.
However, models that included turnover of one or more higher risk group(s) were associated with smaller 
ART prevention impacts compared with models without turnover: 
\x{api/inc/act.turn.any.TRUE.eff}\% IR, \x{api/chi/act.turn.any.TRUE.eff}\% CIA.
Taken together, models that captured heterogenetiy in risk across activity groups and/or key population(s) with turnover were associated with 
reduced ART prevention impacts.
\par
After including risk heterogeneity, further capturing differential ART cascade across activity groups or key populations was associated with differences in projected ART prevention impacts.
In models that had stratifyied by sex/gender, capturing a lower ART cascade among men was associated with a smaller CIA:
...
In the context of risk heterogeneity where key populations were expliciltly modelled, 
including ART cascade prioritized to any key population(s) was associated with an
increased impact and enough to overcome reductions due to turnover:
\x{api/inc/Risk.KP-(priority).eff}\% IR, \x{api/chi/Risk.KP-(priority).eff}\% CIA.
No studies in Dataset~B examined lower ART cascade among key population(s). %SM: key point! (added to abstract)
